...
def format_packs_data_for_installation(args: Dict) -> List[Dict[str, str]]:
    ...
    try:
        ...
        raise Exception(f'The following key does not exist in the pack data: {e}.') from e
    except Exception as e:
        raise Exception(f'Unknown error occurred while processing the packs data.\n{e}') from e
...
def option_handler():
    parser = argparse.ArgumentParser(description="Get MarketPlace Packs list to install")
    # disable-secrets-detection-start
    parser.add_argument('-pl', '--marketplace-packs-list', help="MarketPlace Packs to install", required=True)
...
def main():
    try:
        ...
        packs_data = options.marketplace_packs_list
        args = {"packs_data": packs_data, "pack_id_key": "id", "pack_version_key": "version"}
